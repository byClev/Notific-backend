...
def upsert_news_items(items):
    with APP.app_context():
        ...
        # Fix sequence for Postgres to avoid conflicts
        try:
            ...
            if dialect_name == 'postgresql':
                ...
                if seq_name:
                    # set seq to max(id)
                    max_id = db.session.execute(text('SELECT MAX(id) FROM news')).scalar() or 0
                    setval_sql = text(f"SELECT setval(:seq, :val, true)")
